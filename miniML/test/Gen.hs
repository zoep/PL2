...
genExpSize s = case s of
    ...
    _ -> frequency [ (2, baseCases)
                   ...
                   , (1, liftM3 (Abs nowhere) genVar genOptTypeS genExpS)
                   , (1, liftM3 (ITE nowhere) genExpS genExpS genExpS)
                   , (1, liftM3 (Bop nowhere) genBop genExpS genExpS)
                   , (1, liftM  (Uop nowhere Not) genExpS)
                   , (1, liftM2 (\e1 e2 -> Tuple nowhere [e1, e2]) genExpS genExpS)
                   , (1, liftM  (Proj nowhere 0) genExpS)
                   , (1, liftM  (Proj nowhere 1) genExpS)
                   , (1, liftM2 (Inl nowhere) (Just <$> genTypeS) genExpS)
                   , (1, liftM2 (Inr nowhere) (Just <$> genTypeS) genExpS)
                   , (1, liftM5 (Case nowhere) genExpS genVar genExpS genVar genExpS)
                   , (1, liftM4 (Let nowhere) genVar (Just <$> genTypeS) genExpS genExpS)
                   , (1, do
                           x <- genVar
                           liftM5 (LetRec nowhere x) genVar (Just <$> genTypeS) (Just <$> genTypeS) genExpS genExpS)
                   , (1, liftM (Nil nowhere) (Just <$> genTypeS))
