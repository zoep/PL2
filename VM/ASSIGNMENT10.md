...
MiniML has gotten a brand new [bytecode
compiler](http://www.softlab.ntua.gr/~zoepar/miniML/index.html)! The compiler
compiles MiniML programs to the bytecode described in this assignment. Some
example programs (together with their bytecode) are given in `examples/`. MiniML
has also been extended with the primitives `read_int` and `print_int` so that
MiniML programs can interact with their environment.

The objective of this exercise is to implement a stack-based virtual machine for
this bytecode in Rust. The virtual machine must include a garbage collector to
reclaim dynamically allocated memory that is no longer reachable by the program.
...
bytes representing an integer in *little-endian order*. If the integer is
...
- It copies all heap blocks that are referenced by the root set from the
  ...
  1. If the pointer points to the header of a block in the from-space then the
    ...
    to the to-space, then this is a forwarding pointer. The value of the
...
The [MiniML compiler](http://www.softlab.ntua.gr/~zoepar/miniML/index.html)
compiles MiniML code into the bytecode of the VM. The MiniML language is almost
identical to the one of the previous assignments, but it has been extended to
include the following primitive functions:
- `read_int : () -> int`
  Reads an integer from the standard input.
- `print_int : int -> ()`
  Outputs an integer to standard output.

There are also some minor syntax differences:

- Empty lists are written as `[]` and no type annotations can be given.
- No type annotations can be given at sum types.

The web interface of the compiler outputs the bytecode in Base64 encoding.
...
Submit the files `heap.rs` and `vm.rs` from the `src/` directory to the Helios
submission page. Do not submit zip files. Your files should compile with the
rest of the Cargo project without any modifications to the existing code.
