...
(** In this section we will use Coq to study the semantics of simple
    ...
    In this section, we will also take a deeper dive into Coq's
    tactics, and how we can use Coq's tactic language, Ltac to
...
Example eval_example:
  answer ==> 42.
Proof.
  (* The following to tactics fails, as Coq cannot unify 42 with [n1 * n2].
...
(** Often it is useful to define shorthand for tactics that we use
    ...
    the proof context and goal and automate proofs. For now, we will
    see only few of its capabilities.
    ...
    derived. It generates equalities that state what should be true
    for the arguments of the inductive relations. *)
...
Proof.
  ...
  - inv Heval; simpl.
    ...
    (* Here, we want to rewrite with the conclusion of IHe1.  We can
       ...
       conclusion is an equality. If the statement has premises, Coq
       will generate new goals asking us to prove them. If the
       statement has quantified variables, then Coq will try to unify
       them when matching the left hand side of the equality with a
       subterm of the goal. If it cannot unify them all, it will fail.
       If we can use [erewrite] then Coq will generate evars for the
...
(** Tacticals are "tactic operators" that take other tactics as
    arguments. They are a powerful facility to automate our proofs. *)
...
Proof.
  repeat split; (rewrite <- plus_n_O || rewrite Nat.mul_1_r); reflexivity.
Qed.
...
Definition small_step_stop (e : exp) :
  ~ (exists e', e --> e') ->
  (exists n, e = Num n).
Proof.
  ...
  - (* The rest of the cases are largely similar. We will use them to
    ...
    (* If the result of any logical truth is an existential, we can
       directly use destruct on it. If it has premises, Coq will ask
...
(** We will now formalize the above as an inductive relation in Coq. *)
