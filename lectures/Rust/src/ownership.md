...
So far, we’ve only discussed stack-allocated types. In Rust, a tuple is one of
such stack-allocated types. In the following example, the tuple `t` is allocated
on the stack frame of main and is deallocated once the function returns and its
...
```rust, editable
fn main () {
    let mut vec = Vec::new(); // call to the Vec constructor
...
fn main () {
    ...
    let (sum1, mut vec2) = sum(vec1); // vec2 must be made mutable in order to be able to modify it
