...
Rust provides a reference type that allows borrowing a value without taking
...
With these restrictions, Rust prevents subtle memory errors like iterator
...
fn dangling<'a>() -> &'a Vec<u32> { // 'a is a lifetime variable. We'll discuss this later in detail. You may ignore it for now.
    let mut vec = Vec::new();
    vec.push(42);
    &vec // ERROR: returning reference to a local variable
}

fn main() {
    let v = dangling();
    println!("Will this work? {:?}", v);
}
```
...
lifetime of the variables created within the scope ends.
