...
-- We can also add arguments to the definition
add' :: Num a => a -> a -> a
add' x y = x + y
...
-- A non terminating function. We can define such a function and pass it around
-- without observing any non termination, as long as its value is not explicitly
-- needed
loop :: () -> a
loop _ = loop ()
...
-- We can achieve this with foldl' that has a strict application of the operator.Applicative
